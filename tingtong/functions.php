...
function tt_enqueue_and_localize_scripts() {
	// Wczytaj główny arkusz stylów motywu.
	wp_enqueue_style( 'tt-style', get_stylesheet_uri(), [], '1.0' );

	// Wczytaj główny skrypt aplikacji.
	wp_enqueue_script( 'tt-main-app', get_template_directory_uri() . '/assets/js/main.js', [], '1.0', true );

	// Przekaż dane do skryptu (zalogowanie, slajdy).
	wp_localize_script(
		'tt-main-app',
		'TingTongData',
		[
	...
	// Przekaż dane dla zapytań AJAX (URL i nonce).
	wp_localize_script(
		'tt-main-app',
		'ajax_object',
		[
